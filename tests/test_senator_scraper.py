...
class TestSenatorScraper(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.scraper = SenatorScraper()
        self.scraper.output_dir = Path("data/test_raw/senadores")
    ...
    async def test_download_candidate_data_success(self, mock_fetch_all):
        ...
        success = await self.scraper.download_candidate_data(candidate, sem)
        self.assertTrue(success)
        
        expected_file = self.scraper.output_dir / "12345678.json"
